...
try {
  process.stdout.write(`Retrieving docker build from 680542703984.dkr.ecr.us-east-1.amazonaws.com/project-template:${dockerImageLabel} ...`);
  execSync(`docker pull 680542703984.dkr.ecr.us-east-1.amazonaws.com/project-template:${dockerImageLabel}`);
} catch (err) {
  if (!/manifest.*not found/.test(err.message)) {
    throw err;
  }
  process.stdout.write(`No build cache found for project-template:${dockerImageLabel}, building from scratch.`);
}
execSync(`docker build -f ${join(context, 'Dockerfile')} --cache-from 680542703984.dkr.ecr.us-east-1.amazonaws.com/project-template:${dockerImageLabel} -t project-template ${context}`, { stdio: 'inherit' });
