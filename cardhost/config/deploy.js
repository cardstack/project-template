...
module.exports = function(deployTarget) {
  ...
  if (deployTarget === 'staging') {
    ...
    ENV.s3Assets.bucket = ENV.s3Pages.bucket = 'staging-project-template-web';
  ...
  if (deployTarget === 'sandbox') {
    ENV.build.environment = 'production';
    ENV.s3Assets.bucket = ENV.s3Pages.bucket = 'sandbox-project-template-web';
    ENV.s3Assets.region = ENV.s3Pages.region = 'us-east-1';
  ...
  ENV.slack = {
    ...
    didActivate: function() {
      return function(slack) {
        return slack.notify({
          attachments: [{
            fallback: "Successfully deployed `project-template` commit " + `${sha} to ${targetName} (${deployTarget})`,
            text: "Successfully deployed `project-template` commit " + `${sha} to ${targetName} (${deployTarget})`,
            color: "good"
          }]
        });
      };
    ...
    didFail: function() {
      return function(slack) {
        return slack.notify({
          attachments: [{
            fallback: "Failed `project-template` commit " + `${sha} to ${targetName} (${deployTarget})`,
            text: "Failed `project-template` commit " + `${sha} to ${targetName} (${deployTarget})`,
            color: "danger"
          }]
        });
      };
